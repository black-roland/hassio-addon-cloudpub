...
Поскольку Home Assistant по умолчанию блокирует запросы от прокси-серверов, необходимо разрешить клиенту CloudPub доступ к вашему Home Assistant. Для этого настройте параметры HTTP-сервера через веб-интерфейс:

1. Перейдите в *Настройки* → *Система* → [*Сеть*](https://my.home-assistant.io/redirect/network/).
2. В блоке *HTTP-сервер*, в разделе *Обратный прокси*:
   - Включите *Доверять X-Forwarded-For*;
   - Добавьте доверенный прокси: `172.30.33.0/24`.
3. Нажмите *Сохранить*.

<img height="400" alt="Пример настройки HTTP-сервера Home Assistant" src="https://github.com/user-attachments/assets/cbb57979-d339-4aa3-bc14-d3130f950945" /><br><br>

> [!TIP]
> Обычно нет необходимости изменять адрес доверенного прокси, так как IP-диапазон сети Docker всегда одинаков. Однако, если вы используете нестандартные методы размещения HA (например, Proxmox), то возможно, потребуется указать другой IP-диапазон. Проверьте логи HA после попытки подключения, чтобы найти правильный IP-адрес.
...
### Нужен ли платный тариф для доступа к Home Assistant?

**Нет.** Для удаленного доступа к веб-интерфейсу Home Assistant достаточно бесплатного тарифа CloudPub. Платный PRO-тариф требуется только если вам нужны дополнительные возможности:
- Доступ по MQTT, RTSP, TCP/UDP протоколам.
- Публикация RDP/VNC соединений.
- Использование собственных доменов.
- Безлимитный трафик (на бесплатном тарифе 250 ГБ/месяц).

Актуальную информацию о тарифах можно найти на [официальном сайте CloudPub](https://cloudpub.ru/plans/).
...
#### Проблема с конфигурацией доверенных прокси

Если вы получаете ошибку `400: Bad Request` при попытке открыть ссылку в CloudPub, возможно, настройки доверенных прокси не применились. Убедитесь, что в разделе *Настройки* → *Система* → *Сеть* → *HTTP-сервер* → *Обратный прокси*:
- Включена опция *Доверять X-Forwarded-For*;
- В списке доверенных прокси присутствует `172.30.33.0/24`.

#### Совместимость с Dataplicity

CloudPub можно использовать совместно с интеграцией Dataplicity. Для этого в список доверенных прокси добавьте `127.0.0.1/32` в дополнение к `172.30.33.0/24`.
...
### Превышено время ожидания при выполнении 'clo ls'

Если при старте аддона вы видите эту ошибку, то значит клиент не смог подключиться к серверам CloudPub. Частая причина — блокировка домена `cloudpub.ru` DNS-резолвером с фильтрацией (например, [Quad9](https://quad9.net/result/?url=cloudpub.ru)).

<img width="400" alt="Quad9 Blocked Domain Tester" src="https://github.com/user-attachments/assets/1585358f-9dc0-4344-9350-7480bd3b7999" />

#### Проверка блокировки DNS

Проверить блокировку можно, [выполнив команду ниже](https://github.com/hassio-addons/app-ssh):

```bash
nslookup cloudpub.ru
```

Признаки блокировки: `NXDOMAIN`, пустой ответ или адрес-заглушка вроде `0.0.0.0`.

#### Решение

1. Смените DNS на нефильтрующий. Например, можно переключиться на провайдерский DNS.
2. Либо добавьте `cloudpub.ru` в белый список вашего фильтрующего DNS, если он это поддерживает.
