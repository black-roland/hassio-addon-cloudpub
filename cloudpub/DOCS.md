...
> [!TIP]
> Обычно нет необходимости изменять эти настройки, так как IP-диапазон сети Docker всегда одинаков. Однако, если вы используете нестандартные методы размещения HA (например, Proxmox), возможно, потребуется добавить другой IP-диапазон. Проверьте логи HA после попытки подключения, чтобы найти правильный IP-адрес.
